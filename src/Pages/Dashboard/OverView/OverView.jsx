  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVars} className="flex flex-col gap-2">
        ...
        <p className="text-slate-500 font-medium">Welcome back, {user?.displayName}. Here is what's happening today.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          ...
          >
            ...
            <div className={`h-16 w-16 rounded-2xl ${stat.bg} flex items-center justify-center text-3xl`}>
              {stat.icon}
            </div>
          </motion.div>
        ))}
      ...
      <motion.div variants={itemVars} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-8 flex items-center gap-2">
            <span className="w-2 h-2 bg-emerald-500 rounded-full"></span> Genre Distribution
          </h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800">
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-8 flex items-center gap-2">
            <span className="w-2 h-2 bg-blue-500 rounded-full"></span> Genre Shares
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
      ...
      {/* Table Section */}
      <motion.div variants={itemVars} className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm">
        <div className="px-8 py-6 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex items-center gap-3">
          ...
          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-600 dark:text-slate-300">Detailed Inventory Breakdown</h3>
        </div>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {countGenre.map((genre, i) => (
                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-8 py-4 font-bold text-slate-400">{i + 1}</td>
                  <td className="px-8 py-4 font-black text-slate-700 dark:text-slate-200">{genre._id}</td>
